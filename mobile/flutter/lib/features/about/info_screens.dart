...
class PrivacyPolicyScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const _InfoScaffold(
        ...
        children: [
          ...
          _Heading('Safety'),
          _Body('WVAB is an assistive prototype and is not a certified navigation, medical, emergency, or safety device. Object detection can be wrong or incomplete; always use appropriate mobility and safety practices.'),
          _Heading('Contact'),
...
class OpenSourceLicensesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _InfoScaffold(
        ...
        children: [
          const _Lead('WVAB includes original project code and third-party packages, runtime libraries, fonts, and model assets. Third-party components remain subject to their own licenses.'),
          const _Heading('WVAB source code'),
          const _Body('Original WVAB repository code is provided under the MIT License. Copyright © 2026 Shahanur Islam.'),
          const _Heading('Ultralytics YOLOv8 model asset'),
          const _Body('The bundled YOLOv8n model is third-party material. The project’s third-party notice records Ultralytics open-source software/model licensing as AGPL-3.0 by default, with separate commercial/Enterprise licensing options. WVAB’s MIT license does not relicense that model or other third-party material.'),
          const _Heading('ONNX Runtime'),
          const _Body('WVAB uses ONNX Runtime on Android for local inference. ONNX Runtime is third-party software distributed under the MIT License.'),
          const _Heading('Flutter packages and transitive dependencies'),
          const _Body('Flutter, Dart packages, Android libraries, and their transitive dependencies keep their own upstream license terms. Use the button below to inspect the license registry included in this installed build.'),
          const SizedBox(height: 8),
          FilledButton.icon(
            key: Key('flutter-license-page-button'),
            onPressed: null,
            icon: Icon(Icons.description_outlined),
            label: Text('Open package license registry'),
          ),
        ],
...
class HowItWorksScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const _InfoScaffold(
        title: 'How It Works',
        children: [
          _Lead('WVAB turns camera frames into local object detections and spoken/haptic guidance without requiring a cloud inference server for phone-camera mode.'),
          ...
          _Step('4', 'Filter and locate', 'WVAB applies your confidence threshold and class selection, then estimates relative proximity from bounding-box size and left/center/right position from the box center.'),
          _Step('5', 'Speak exact objects', 'Up to three distinct current objects are selected by proximity and confidence. Spoken guidance uses the detected label, proximity, and direction instead of forcing “person” as the top class.'),
          _Step('6', 'Store local history', 'Recent detected-object events are stored locally with timestamp, label, confidence, proximity, direction, and camera source. Camera images are not intentionally saved in history.'),
