"""Secure UDP runtime for WVAB.

The public compatibility entrypoint remains ``udp_streaming.py``. This module
owns transport/session state so protocol/security logic can be tested without
turning the repository root script into a monolith.
"""
...
def _load_crypto():
    try:
        ...
        raise RuntimeError("PyCryptodome is required for encrypted UDP") from exc
...
def _secure_transport_settings():
    ...
    if (not encrypt or not require_auth) and not allow_insecure:
        raise RuntimeError(
            ...
            "set WVAB_ALLOW_INSECURE_UDP=1 only for isolated development"
        )
    key = _validate_aes_key(_load_udp_key()) if encrypt else None
    crypto = _load_crypto() if encrypt else (None, None)
    if encrypt and key is None:
        raise RuntimeError("UDP encryption requires a valid 16/24/32-byte AES key")
    ...
    return encrypt, key, require_auth, token, crypto
...
class SimpleTracker:
    def __init__(
        self,
        iou_threshold=TRACK_IOU_DEFAULT,
        max_age_s=TRACK_MAX_AGE_S_DEFAULT,
        min_hits=TRACK_MIN_HITS_DEFAULT,
    ):
        self.iou_threshold = float(iou_threshold)
        if not 0.0 <= self.iou_threshold <= 1.0:
            raise ValueError("tracker IoU threshold must be between 0 and 1")
...
class UDPVisionServer:
    def __init__(
        ...
        (
            ...
            self.require_auth,
            self.auth_token,
            (self.AES, self.get_random_bytes),
        ...
        self.seen_auth_nonces = OrderedDict()
        ...
        if self.language not in self.available_languages:
            self.language = "en"
        self.overlay_font = _load_overlay_font(self.language, self.logger, self.ImageFont)
        ...
        self.announcement_cooldown = max(
            float(os.environ.get("WVAB_UDP_ANNOUNCE_COOLDOWN_S", "2.5")), 0.1
        )
        ...
        self.health_interval_s = max(
            float(os.environ.get("WVAB_UDP_HEALTH_INTERVAL_S", HEALTH_INTERVAL_DEFAULT_S)), 0.5
        )
        ...
        if (
            ...
            and self.control_host not in {"127.0.0.1", "::1", "localhost"}
            and len(dedicated_ws_token) < 16
        ):
    ...
    @staticmethod
    def _remember_bounded(store: OrderedDict, key, limit: int):
        store[key] = time.monotonic()
        store.move_to_end(key)
        while len(store) > limit:
            store.popitem(last=False)

    def _retire_session(self, addr, session_id):
        self._remember_bounded(self.retired_sessions, (addr, session_id), MAX_RETIRED_SESSIONS)
        self.last_completed_frame.pop((addr, session_id), None)

    def _bind_session(self, addr, session_id, now):
        current = self.sessions.get(addr)
        if current is not None and current["session_id"] != session_id:
            self._retire_session(addr, current["session_id"])
        if (addr, session_id) in self.retired_sessions:
            return False
        if current is None and len(self.sessions) >= MAX_AUTH_CLIENTS:
            oldest_addr = min(self.sessions, key=lambda key: self.sessions[key]["last_seen"])
            old = self.sessions.pop(oldest_addr)
            self._retire_session(oldest_addr, old["session_id"])
        self.sessions[addr] = {
    ...
    def _is_authed(self, addr, session_id) -> bool:
        if not self.require_auth:
            return True
        state = self.sessions.get(addr)
        if state is None or state["session_id"] != session_id:
            return False
        if time.monotonic() - state["authenticated_at"] > self.auth_ttl_s:
            return False
    ...
    def _handle_auth(self, session_id: int, header: bytes, payload: bytes, addr) -> bool:
        ...
        if base_nonce is not None:
            self._remember_bounded(self.seen_auth_nonces, base_nonce, MAX_AUTH_NONCES)
        self.last_valid_packet_mono = now
        return True
    ...
    def _cleanup_state(self, frame_buffers):
        ...
        stale_clients = [
            addr
            for addr, state in self.sessions.items()
            if now - state["last_seen"] > self.auth_ttl_s * 10
        ]
        for addr in stale_clients:
            state = self.sessions.pop(addr)
            self._retire_session(addr, state["session_id"])

    @staticmethod
    def _evict_excess_frames(frame_buffers, addr, session_id):
        keys = [key for key in frame_buffers if key[0] == addr and key[1] == session_id]
    ...
    def _set_language(self, language):
        ...
        self.overlay_font = _load_overlay_font(language, self.logger, self.ImageFont)
    ...
    def _control_main(self):
        async def runner():
            ...
            async with websockets.serve(
                ...
                self.logger.info(
                    "WebSocket control listening on %s:%s",
                    self.control_host,
                    self.control_port,
                )
    ...
    def _draw_text(self, frame, text, x, y, color):
        x, y = int(max(x, 2)), int(max(y, 12))
        if self.overlay_font is None:
            self.cv2.putText(
                frame,
                text if str(text).isascii() else "object",
                (x, y),
        ...
        return self.cv2.cvtColor(self.np.asarray(image), self.cv2.COLOR_RGB2BGR)

    @staticmethod
    def _direction(bbox, frame_shape):
        center = (float(bbox[0]) + float(bbox[2])) * 0.5
        width = float(frame_shape[1])
        return "left" if center < width * 0.3 else "right" if center > width * 0.7 else "center"
    ...
    def _process_frame(self, frame):
        ...
        for track in tracks:
            ...
            bbox = track["bbox"]
            direction = self._direction(bbox, frame.shape)
            ...
            while not stop_event.is_set():
                ...
                _write_health(
                    self.health_path,
                    {
                        ...
                        "authenticated_clients": len(self.sessions),
                    },
                    self.logger,
                )
        ...
        try:
            while not stop_event.is_set():
                ...
                if packet is not None:
                    ...
                    try:
                        session_id, frame_id, total_chunks, chunk_index, payload_size = unpack_header(header)
                    except ValueError:
                        continue
                    ...
                    if frame_id == AUTH_FRAME_ID and total_chunks == 0 and chunk_index == 0:
                        if payload_size == len(payload) and payload_size > 0:
                            previous_session = self.sessions.get(addr, {}).get("session_id")
                            if self._handle_auth(session_id, header, payload, addr):
                                if previous_session is not None and previous_session != session_id:
                                    for stale_key in list(frame_buffers):
                                        if stale_key[0] == addr and stale_key[1] != session_id:
                                            frame_buffers.pop(stale_key, None)
                        continue

                    if not valid_frame_shape(total_chunks, chunk_index, payload_size, len(packet)):
                        continue
                    if not self._is_authed(addr, session_id):
                        continue
                    replay_key = (addr, session_id)
                    if not frame_id_is_newer(frame_id, self.last_completed_frame.get(replay_key)):
                        continue

                    key = (addr, session_id, frame_id)
                    entry = frame_buffers.get(key)
                    if entry is None:
                        self._evict_excess_frames(frame_buffers, addr, session_id)
                        entry = {
                            "total": total_chunks,
                            "chunks": {},
                    ...
                    if len(entry["chunks"]) == total_chunks:
                        try:
                            ...
                            continue
                        frame_buffers.pop(key, None)
                        self.last_completed_frame[replay_key] = frame_id
                        for stale_key in list(frame_buffers):
                            if (
                                stale_key[0] == addr
                                and stale_key[1] == session_id
                                and not frame_id_is_newer(stale_key[2], frame_id)
                            ):
                                frame_buffers.pop(stale_key, None)

                        frame = self.cv2.imdecode(
                            self.np.frombuffer(data, dtype=self.np.uint8),
                        ...
                        if not self.headless:
                            ...
                            self.cv2.imshow("WVAB - UDP Server", processed)
...
class UDPCameraClient:
    def __init__(self, server_ip="192.168.4.1", server_port=9999):
        ...
        self.watchdog_check_s = max(
            float(os.environ.get("WVAB_UDP_WATCHDOG_CHECK_S", WATCHDOG_CHECK_DEFAULT_S)), 0.2
        )
        self.watchdog_client_send_idle_s = float(
            os.environ.get(
                "WVAB_UDP_WATCHDOG_CLIENT_SEND_IDLE_S",
    ...
    def send_frames(self, camera_source=0):
        ...
        cap = self.cv2.VideoCapture(camera_source)
        if not cap.isOpened():
        ...
        try:
            while not stop_event.is_set():
                ...
                frame = self.cv2.resize(frame, (640, 360))
                ...
                base_nonce = self.get_random_bytes(NONCE_SIZE) if self.encrypt_udp else None
                ...
                if not _bool_env("WVAB_UDP_CLIENT_HEADLESS", "0"):
                    self.cv2.imshow("WVAB - Camera Client", frame)
                    if self.cv2.waitKey(1) & 0xFF == ord("q"):
                        stop_event.set()
        finally:
            stop_event.set()
